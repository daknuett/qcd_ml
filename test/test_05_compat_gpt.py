...
try:
    ...
    def test_lattice2array2lattice5d():
        grid = g.grid([12, 4, 4, 4, 8], g.double)
    ...
    def test_array2lattice2array5d():
        grid = g.grid([12, 4, 4, 4, 8], g.double)
    ...
    def test_lattice2array5d():
        grid = g.grid([12, 4, 4, 4, 8], g.double)
        value = np.array([[1, 2, 0], [3, 4 + 8j, 5 + 8j], [6, 7+ 3j, 8], [9, 10, 11 + 4j]])

        psi = g.vspincolor(grid)
        psi[:] = 0
        psi[5, 1, 2, 3, 4] = g.vspincolor(value)

        ndarray = lattice2ndarray(psi)

        expect = np.zeros((12, 4, 4, 4, 8, 4, 3), dtype=np.complex128)
        expect[5, 1, 2, 3, 4] = value

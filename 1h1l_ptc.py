...
layer.weights.data[:,:,0] += torch.eye(4)
...
for t in range(training_epochs):
    ...
    if t % check_every == 0:
        with torch.no_grad():
            x_p, ret_p = qcd_ml.util.solver.GMRES_restarted(w, test_v, test_v, prec=lambda v: layer.forward(torch.stack([v]))[0], eps=1e-4, maxiter_inner=25, max_restart=400)
...
torch.save(list(layer.parameters()), "1h1l_ptc.pt")
